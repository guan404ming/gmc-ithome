...
到昨天為止，kernel 已經會生了。GPU 的 Triton 和 CPU 的 C++ 兩條 codegen 都走完，一張 ATen 圖進來，一份能跑的 kernel 出去，整條產線看起來已經完工。但有個問題一直被按著沒問，同一個 op 常常不只一種生法，生哪一種才最快。這個問題編譯器用紙筆推不出來，答案跟 shape、dtype、GPU 型號全都有關，唯一可靠的辦法是每一種都真的跑一次，讓碼表說話。今天就來看 Inductor 的 autotune 機制，以及把它火力全開的 mode="max-autotune"，順便看看這場比賽的門票錢花在哪裡、值不值得。
...
這就是預設模式的哲學，matmul 交給 cuBLAS 準沒錯，編譯快，效能通常也不差。
...
`max_autotune` 讓 GEMM template 進場，`coordinate_descent_tuning` 待會講，`triton.cudagraphs` 是另一個獨立的加速機制，跟今天的主題無關，後面的實驗為了單純用 max-autotune-no-cudagraphs 這個變體隔開它。同一顆 matmul 重編一次，log 裡多了一張計分表（節錄）。
...
有些 config 要求的 shared memory 超過 L40S 的上限，這種候選直接被踢出名單，比賽照常進行，這也是實測的好處，跑不起來的方案自己淘汰自己。

結局有點反高潮，冠軍是 `mm` 本人，cuBLAS 贏了，最快的 Triton 候選差了不到 3%。所以生成的程式碼跟預設模式一模一樣，還是 `extern_kernels.mm`，三方 benchmark 也印證了這件事。
...
方正的 2048 fp16 matmul 是 cuBLAS 的主場，這個結果完全合理。autotune 不是 Triton 必勝的儀式，它是實測，實測的意思是 cuBLAS 該贏的時候就讓它贏，花掉的編譯時間買到的是一句「確認過了」。
...
1.48 倍。cuBLAS 對這種瘦長 shape 沒有特調，模板搜出來的 kernel 就有利可圖，LLM 推理裡大量的小 batch matmul 正是這種體質，max-autotune 在這類工作負載上的口碑也多半是這樣賺來的。同一段程式碼，兩種 shape，兩個完全相反的冠軍，這就是為什麼這件事只能實測。
...
max-autotune 之下，matmul 由模板生成，`relu` 作為 epilogue 直接縫進模板尾巴，兩顆 kernel 變一顆。
...
XBLOCK 往 16 和 4 各試一步，R0_BLOCK、num_warps 也各自上下探過，這一題鄰居全都沒有更快，最後的 1.000x 表示原配就是最佳解。碼表說不用改，也是一種答案。
