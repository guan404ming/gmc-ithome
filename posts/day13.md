...
它做的事簡單來說就是：我們可以想像有一個有潔癖的書記官。你交給他一份手稿，上面滿是「把第三行塗掉改成這樣」「這一段直接畫線刪掉」的in-place 修改指示。他從頭到尾不在你的原稿上動一筆，不過他每次修改都謄到一張新的紙上，然後在帳本上記著「目前最新的版本是哪一張」。等整份文件定稿，他才一次性把最終版謄回你的原稿。原稿的主人看到的結果跟自己動手改一模一樣，但抄寫的過程中沒有任何一張紙被塗改過。

Functionalization 做的就是這件事，只是手稿換成了 Tensor。`x.add_(1)` in-place 改記憶體、`y = x.view(2, 8)` 讓兩個名字共享同一塊儲存，改 `y` 等於改 `x`。這些 mutation 和 aliasing 對後端是毒藥，而 Functionalization 把它們全部謄寫成純函數式的版本，語意留到邊界再一次結清。
...
In-place 直接毀掉這個前提。`add_` 執行之後，所有指向同一塊記憶體的名字通通變了值，而「哪些名字指向同一塊記憶體」這件事，圖上根本看不出來。`y = x.view(2, 8)` 之後，`y` 和 `x` 是兩個節點，儲存卻是同一塊。這時候 `y.add_(1)` 改的不只是 `y`，連 `x` 也一起變了。後端如果想把某個讀 `x` 的運算往前搬，搬過了 `add_` 這條線，答案就錯了。要安全地重排，每個後端都得自己做一套完整的 aliasing 分析，而這種分析聽起來就是一件超麻煩的事。

那能不能叫使用者別寫 in-place？不行。PyTorch 的 API 裡幾乎每個 op 都有帶底線的 in-place 版本，`optimizer.step()` 更新參數靠的全是 in-place，這是 eager 世界的一等公民，編譯器只能自己想辦法。
...
2. **View 用 replay 維持一致**：透過 view 改資料時，把修改反映回 base（把新值 view 回 base 的形狀），再從 base 重新長出 view。base 和 view 永遠指著同一份最新內容，alias 語意靠 replay 保住，而不是靠真的共享記憶體。

兩條規則合起來，圖內部就完全沒有 mutation 了，每個節點的值一旦產生就不再改變，是教科書定義的 SSA 形式。剩下的唯一問題是邊界，這個下面會講。

## 實際測 view 加 in-place

拿一個把 view 和 in-place 都用上的函式來實測，一樣跑在 Modal 的 L40S 上，`torch 2.8.0+cu128`。這個函式透過 view in-place 改了 `x`，最後回傳依賴被改過的 `x`。
...
- 最關鍵的是 `mul` 那行。`x * 2` 用的是 `view_3`，不是原始輸入 `arg0_1`。書記官的帳本記著「`x` 的最新值是 `view_3`」，所以讀 `x` 的地方自動改讀新值。這就是「rebind」，舊名字不再出現，所有引用都指向最新的那份。
...
*圖一：書記官的工作流程。左邊是使用者寫的 in-place 程式，被處理到的 instruction 逐條劃掉、標上改寫後的名字。中間是 FunctionalTensor 的帳本，`x` 和 `y` 兩個名字接在同一條 storage 線上，每次 in-place 改都讓帳本的最新值換一格。右邊是 functionalized 的圖一行一行長出來，`add_` 變成 `add` 加兩條 view replay，結尾補上寫回輸入的 `copy_`。*
...
一個更小的例子看得更清楚。`g` 直接 in-place 改參數。
