...
`mul_` 變 `mul`，`x + 1` 改吃 `mul`，對輸入的修改壓縮成結尾一條 `copy_`。還有一個容易錯過的細節。`aot_graphs` 的 log 在圖前面印了一大串 `ViewAndMutationMeta`，裡面把這件事寫成了 metadata：這個輸入 `mutates_data=True`、整張圖 `keep_input_mutations=True`。AOTAutograd 對每個輸入輸出都記著「它有沒有被改、是不是別人的 alias」，這份 meta 決定了 mutation 是像這樣留在圖尾端，還是搬到圖外由 wrapper 補做。
