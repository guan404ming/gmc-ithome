...
先看交接現場。整個系列的實驗一樣跑在 Modal 的 L40S、PyTorch 2.8.0 上，拿一個最小的訓練形狀：
...
最後一行輸出是 `w.grad shape: torch.Size([4, 4])`，梯度真的算出來了。這兩張圖裡有好幾個值得放大來一起看看的地方。
...
**順帶一提**，backward 圖的第一個 op 是 `expand`：把純量的 `tangents_1` 撐成 `[4, 4]`。這就是 `sum` 的導數，每個元素對 loss 的貢獻都是 1，梯度就是上游梯度廣播到整個 shape。你平常呼叫 `loss.backward()` 時 autograd 引擎默默做的事，在這裡全部變成了圖上可以指著看的節點。

## 對照組：包進 no_grad 會發生什麼事？
...
要不要展開 backward，是看輸入的 `requires_grad` 和當下的 grad mode 決定的。這邊也回收了 Day 6 的一個伏筆：`GLOBAL_STATE grad_mode` 那條 Guard 之所以存在，就是因為開著 grad 編出來的圖和 `no_grad` 底下編的根本是兩個東西，grad mode 一變，整套編譯產物都得換。
...
順帶發生的還有兩件事，各佔一天：in-place 和 view 被改寫成純函數式（Functionalization，明天），高階 op 被拆成更小的基本運算（Decomposition，後天）。所以 Inductor 拿到的圖，跟你寫的 Python 已經隔了好幾層轉換，但每一層都留著 log 可以對照，這也是這個系列一路 dump 中間產物的底氣。
...
最後把「為什麼要這麼大費周章」講清楚。Eager 的 autograd 是 define-by-run：forward 跑到哪、tape 記到哪，backward 按 tape 回放。好處是彈性，代價是 backward 永遠是一連串分開的 kernel，沒有全局可看、沒有 fusion 可做。AOT 展開之後：
...
明天拆推演過程中的第一層轉換：Functionalization。`x.add_(1)`、view 這些會就地改記憶體的操作，是怎麼被改寫成純函數式、又保證語意不變的。那我們明天見！
