...
昨天把重編修好了，更早之前也學會了怎麼繞開 graph break，但這些診斷手法一直是邊用邊講，用到哪把撿哪把。實務上出事的當下最需要的其實是一張清單，慢了先開哪個 log、越跑越慢先查哪一項、編譯直接掛掉又該交出什麼給 issue。所以今天把整個系列用過的診斷工具一次收齊，整理成一個出事的時候照著按的工具箱。比喻是診間的檢查流程，log 是問診紀錄，explain 是健檢總表，depyf 是 X 光片，minifier 則是最後才上場的手術刀。症狀走一遍流程，每站留下一份證據，病灶自己會浮出來。本篇實驗全部在本機 CPU 上跑（torch 2.8.0），完整程式與 log 在 `code/day27/`。
...
表的順序就是排查的順序。break 決定圖有幾張，recompile 決定同一張圖付了幾次編譯費，這兩項是成本的大頭，都確認沒問題才輪得到懷疑 kernel 本身的品質。實驗品是同一個小函式，故意埋一個資料相依的分支，等一下每把工具都拿它開刀。
...
順帶一提，前三列的 `TORCH_LOGS` 不是三個獨立開關，它是整條 pipeline 共用的日誌系統，每個元件把自己的產出註冊成一種 artifact，名字全登記在 [`torch/_logging/_registrations.py`](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/_logging/_registrations.py)，用逗號隔開可以一次點好幾道。這個系列每次實驗開場那行環境變數，點的就是這份菜單上的菜。七把工具的使用成本也不一樣，`TORCH_LOGS` 這一族完全不用改程式，設個環境變數重跑一次就有，explain 要改一行呼叫方式，depyf 得額外安裝，所以排查總是從最便宜的 log 開起。log 量大到讀不動的時候，官方還有一個叫 tlparse 的整理工具能把結構化紀錄收成報告，本篇就不展開了。

## 先數 break

排查的第一步永遠是數 break，因為它決定了你手上到底有幾張圖，而圖的碎片數是後面一切成本的基數。開 `TORCH_LOGS="graph_breaks"` 編譯一次（節錄）。
...
break 數完換 recompile。讓輸入從 32x32 換成 48x48 再換成 64x64，開 `TORCH_LOGS="recompiles"`（節錄）。
...
前兩站看的是編譯器的決定，要看它真的生出什麼，開 `TORCH_LOGS="output_code"`（節錄）。
...
整個 graph break 的機制濃縮在六行裡。前半段的計算全被收進一個 compiled function，連分支要用的布林值都是它的輸出之一，`if` 本身留在 Python，兩條路各自接一個 resume function。dump 目錄裡每個 compiled function 從 FX Graph、ATen 圖到 kernel 都各有一份檔案，猜測和現實對不上的時候，這疊 X 光片可以一層一層對下去，比對著十六進位的 bytecode 用肉眼考古省力得多。
...
前面的工具處理的都是「不夠快」，minifier 處理的是「掛掉了」。真實模型幾百個 op，編譯途中丟出例外時，不可能把整個模型貼進 issue，得先把圖削到最小。設 `TORCHDYNAMO_REPRO_AFTER="dynamo"` 之後，backend 一丟例外，Dynamo 就把當下那張圖連同輸入打包成一支重現腳本，這條路的實作在 [`torch/_dynamo/repro/after_dynamo.py`](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/_dynamo/repro/after_dynamo.py)。示範的方式是故意寫一個看到 `relu` 就翻臉的 backend，拿一個五個 op 的小函式去撞。
...
launcher 裡裝的是完整的犯罪現場，出事那張圖被重建成一個獨立的 module，連當時的輸入 tensor 都一併存檔。跑這支 launcher，它會拿著同一個 backend 反覆實驗，每輪砍掉一部分 node，錯誤還在就繼續砍，錯誤消失就把 node 放回去換個方向再砍（節錄）。
...
實驗裡的小圖四輪就砍到底了，最後吐出的 `repro.py` 是一支可以獨立執行的腳本，圖的部分只剩一行計算。
...
`sin`、`cos` 和加法乘法全被證明無辜，兇手就是 `relu`，這份腳本可以直接附進 bug report，維護者不需要你的模型和資料就能重現。它還有幾種本篇沒有實跑的模式，設成 `"aot"` 對付 AOTAutograd 之後才爆炸的案子，搭配 repro level 4 的話，連編譯結果數值不對這種 accuracy bug 都能用同一套流程自動縮小。
