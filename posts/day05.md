...
昨天 `InstructionTranslator` 的 stack 上放的東西，一直被含糊地叫「符號替身」。今天把它講清楚。在 [`torch/_dynamo/variables/`](https://github.com/pytorch/pytorch/tree/v2.8.0/torch/_dynamo/variables) 底下，Dynamo 幫追蹤時可能碰到的每一種 Python 值都準備了一個包裝類別，統稱 `VariableTracker`。這套型別系統，就是 Dynamo 對「Python 到底有多動態」這個問題的完整回答。

搭配它的還有一個小東西叫 `Source`，記的是「這個值在 runtime 要怎麼拿到」。理解這兩個，你就會懂三件昨天留下的懸案：Python 的 int 為什麼會被 bake 進圖變成常數、呼叫自己寫的函式為什麼不會 Graph Break、以及 Guard 到底是從哪裡長出來的。
...
*圖一：`f(x, n, items, cfg)` 的每個輸入怎麼被包。左邊是進來的值和它的 Source，中間是對應的 `VariableTracker` 與它回答問題的方式，右邊是結果：Tensor 進圖當輸入，int、list 元素、物件屬性被 bake 成常數，自己寫的函式被 inline，每一列都附帶一條 Guard。*

## 為什麼需要包

Dynamo 在翻譯 bytecode 時手上沒有真值，但指令的行為偏偏取決於值的型別：同樣一條 `CALL`，呼叫的是 `torch.sin` 還是你自己寫的函式，處理方式天差地遠；同樣一條 `BINARY_OP *`，兩邊是 Tensor 還是 int，一個要進圖、一個要當場算掉。所以每個值都需要一個「知道自己是什麼」的替身，handler 才能問它：你被乘的時候怎麼辦？你被呼叫的時候怎麼辦？你被取屬性的時候怎麼辦？
...
`variables/` 底下有幾十個檔案、上百個類別，但你最常撞到的就這幾個，它們的行為差異就是 Dynamo 行為的縮影：
...
圖因此更好最佳化，常數可以摺疊、可以特化，Inductor 生 kernel 時 `* 3` 是一個立即數而不是一次記憶體讀取。代價是這張圖只對 `n == 3`、`items[0] == 1`、`cfg.scale == 2` 成立。誰來記這筆帳？就是 Guard，下面會看到。

Tensor 的數值反過來永遠留作 runtime 輸入，Dynamo 只特化 shape 和 dtype。這是一個關於「變化頻率」的賭注：Tensor 的數值每個 batch 都不同，bake 進圖等於每次呼叫都重編，划不來；int 純量常直接決定生成程式碼的長相（迴圈邊界、索引、分支），bake 進去收益大，而 Dynamo 賭它通常不會變。賭輸了就是一次 Recompile。同一個位置變太多次，Dynamo 會自動把它升級成符號整數停損，這是 Symbolic Shapes 那篇的事。

## UserFunctionVariable：呼叫不是斷點，是入口

再看 trace 裡 `CALL 1` 之後那幾行：stack 突然清空、出現 `RESUME`、`LOAD_FAST t`，這是 `helper` 的 bytecode。Dynamo 碰到呼叫你自己寫的函式，不會斷圖，而是開一個 `InliningInstructionTranslator`（昨天那台機器的子類別，同一份 `symbolic_convert.py`），鑽進被呼叫函式的 bytecode 繼續翻譯，翻完把回傳值接回原本的 stack。所以圖上 `y = L_x_ * 3` 那行的來源註解是 `in helper`，不是 `in f`：整條呼叫鏈被攤平成一張圖。
...
順帶一提 `LazyVariableTracker`：`x`、`n`、`items`、`cfg` 剛被 `LOAD_FAST` 推上 stack 時全都是它。包裝本身有成本（要建 Guard、要查型別、要遞迴包元素），一個從沒被碰過的值不值得展開，所以先掛個懶惰的殼，真的被用到那一刻才實體化。`x` 是在進到 `helper` 裡碰到 `BINARY_OP` 才變成 `TensorVariable` 的。編譯時間就是這樣省下來的。

## Source：這個值是從哪裡來的

`VariableTracker` 還有一個關鍵欄位 `source`，回答另一個問題：這個值在 runtime 要怎麼拿到。定義在 [`source.py`](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/_dynamo/source.py)，用起來像這樣：
...
根節點只有兩種：`LocalSource`（frame 的區域變數，印成 `L[...]`）和 `GlobalSource`（全域，印成 `G[...]`），後面用 `AttrSource`、`GetItemSource` 一路鏈下去，任何值的取得路徑都能表示。這條鏈有兩個用途，正好對應 Dynamo 的兩個出口：

- `source.make_guard(...)`：生成 Guard。`cfg.scale` 被 bake 成常數 2，前提「`L['cfg'].scale == 2`」就得記下來，下次呼叫時檢查。
- `source.reconstruct(codegen)`：生成 bytecode。改寫後的 bytecode 要把圖的輸入準備好，靠的就是照著這條鏈發出 `LOAD_FAST cfg`、`LOAD_ATTR scale` 這樣的指令把值取出來。
...
每一行左邊的 `source=` 就是 Source 的 `name()`。看得出前面的帳全在這裡：`n`、`items[0]`、`cfg.scale` 三個被 bake 的常數各有一條 `EQUALS_MATCH`；`x` 只守 dtype、shape、stride，不守數值（`TENSOR_MATCH`）；被 inline 的 `helper` 守的是它的 `__code__` 物件 id，你重新定義 `helper` 它就會發現。實際改一下值：
...
一句話總結這個分流：Guard 守的是圖跟外面世界的邊界，不是圖的內部。

## 結語

`VariableTracker` 是 Dynamo 的型別系統：每一種 Python 值都有一個知道「自己被呼叫、被取屬性、被運算時該怎麼辦」的替身。Tensor 進圖，純量 bake 成常數，你寫的函式被 inline 攤平，其他物件逐屬性追蹤。`Source` 是每個替身身上的地址，記著它在 runtime 怎麼拿，一頭生出 Guard，一頭生出重建它的 bytecode。有 Source 的值走 `VariableBuilder` 並裝 Guard，追蹤中途生出的值走 `SourcelessBuilder` 不裝。

`cfg.scale` 被 bake 成常數的代價，是一條「`L['cfg'].scale` 必須還是 2」的前提。今天看到前提從哪裡長出來，明天看它長什麼樣、怎麼被檢查：Guard 有哪幾種、`TORCH_LOGS=guards` 那棵樹的每一行怎麼讀、以及為了讓每次呼叫的檢查夠快，Dynamo 把整套 Guard 編成了一棵 C++ 的樹。那我們明天見！
