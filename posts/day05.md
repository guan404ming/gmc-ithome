# Day 5 | TorchDynamo 裡有替身使者？ VariableTracker 登場！
...
`cfg.scale` 也就是常數被 bake 成常數的代價，是一個「`L['cfg'].scale` 必須還是 2」的前提。今天我們看到這個前提從哪裡長出來，明天就會來看看它是長什麼樣子、怎麼樣被檢查：Guard 有哪幾種、`TORCH_LOGS=guards` 那棵樹的每一行怎麼讀，以及為了讓每次呼叫的檢查夠快，Dynamo 這邊做了什麼酷酷的操作。那一樣我們明天見！
