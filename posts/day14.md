...
在拆之前，先把「op 有幾層」這件事講清楚，因為 decomposition 拆的方向就是沿著這個階層往下走：
...
1123 條規則（比 827 多是因為表是以 overload 為單位登記的）。每一條規則就是「用其他 op 把這個 op 實作一遍」的普通 Python 函式，用一個 decorator 註冊進表裡。GELU 那條就寫在 [`torch/_refs/nn/functional/__init__.py`](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/_refs/nn/functional/__init__.py)，把包裝拿掉之後核心長這樣：
...
- [torch/_refs/nn/functional/__init__.py：GELU 的拆解規則（v2.8.0）](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/_refs/nn/functional/__init__.py)
...
- [IRs：Core ATen IR 與 Prims IR（PyTorch 官方文件）](https://pytorch.org/docs/stable/torch.compiler_ir.html)
- [PyTorch 2.0 發布公告：PrimTorch 一節](https://pytorch.org/get-started/pytorch-2.0/)
