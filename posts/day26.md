...
這串清單不是免費的。圖越養越多，命中前要驗的 Guard 也越多。所以 recompile 爆炸的帳有兩筆，比較容易察覺的的是每次 recompile 的編譯費，而另一種就是就算最後命中了，每次呼叫都得先白白驗一輪，執行期的固定開銷又被拉長。
...
再拿 step=999 呼叫也只花 0.17 ms，答案還是正確的，因為整個函式已經退回 eager，加速歸零。這一切只出現在 log 裡，程式安靜得像沒事，正是 recompile 爆炸最難以察覺的地方。
...
shape 那一類還有一個容易漏掉的角落。batch 裡混進 1 筆的時候，明明是 dynamic 的圖還是又編一張，log 講得很清楚（節錄）。
...
grad mode 這種有界的 recompile 不用修，兩張圖共存本來就是 cache 的正常用法，值得動手的是每次呼叫必炸的那種。保險絲也可以調，`recompile_limit` 改大能多撐幾張圖，但只是延後爆炸，因為多大的櫃子都會滿。
...
今天靠一個 log 開關就破了案，但 torch.compile 的疑難雜症不只 recompile，圖被切碎、結果不對、速度沒變快，各有各的查法。明天就把除錯工具箱攤開，TORCH_LOGS 全家桶、tlparse、minifier，看看官方配了哪些好用的傢伙在工具箱裡面。那我們明天見！
