...
昨天的結尾留了一句「shape 要穩定」。不只 CUDA Graph 怕變動，整個 torch.compile 都建立在「這張圖的假設還成立」上。假設一塌，Guard 驗票失敗，Dynamo 就再編一張圖。偶爾一次是機制正常，每次呼叫都來就是事故，加速全被編譯本身吃掉。更麻煩的是撞到上限後整個函式會默默退回 eager，一句錯誤訊息都沒有。今天就來處理這場事故，重編怎麼發生、怎麼用 log 找到兇手、怎麼對症下藥。
...
這串清單不是免費的。圖越養越多，命中前要白驗的 Guard 也越多。所以重編爆炸的帳有兩筆，明著的是編譯費，暗著的是就算命中，昨天才壓下去的固定開銷又被拉長。
...
Dynamo 把從 global 讀進來的 Python 數值當常數烙進圖裡，也就是對這個值特化，Guard 記下它必須等於 0。step 變成 1 就驗票失敗，再編一張烙著 1 的圖，然後是 2、是 3，每張圖只服務一個值。時間帳單長這樣。
...
前八次呼叫總共付了 10.19 秒，養出八張圖，每張只用過一次。這還是一行的玩具函式，真實模型的一次冷編譯貴得多。然後 call 8 突然只剩 4 ms，不是修好了，是保險絲斷了。
...
同一份程式再跑幾組對照，把常見的爆炸源分成四類。分類看的是同一個問題，變動的那樣東西有沒有人接得住。

- **Python 常數**：沒人接。從 global 撿來的值會被烙進圖裡，一個值一張圖，就是剛剛炸掉的原因。同樣的值改成當引數傳進來就有 automatic dynamic 接手，純量升級成符號，十個值只重編一次。
...
- **函式身分**：沒人接。把函式當引數傳給編譯過的函式，每次傳一個內容不同的 lambda 都會觸發 `___check_obj_id` 這種失敗。Guard 驗的是 code object 的身分，迴圈裡重建同一行 lambda 不會炸，字面上不同的函式才是一個一張圖。

shape 那一類還有一個容易漏掉的角落。batch 裡混進 1 筆的時候，明明是 dynamic 的圖還是又編一張，log 講得很白（節錄）。
...
講 dynamic shape 時提過，0 和 1 永遠被特化，符號的下界是 2。最後一個 batch 落單剩 1 筆，就會多養一張圖。
...
- **會變的 shape 事先標 dynamic**：用 `mark_dynamic` 講明白，六種 batch 從頭到尾一張圖，連 automatic dynamic 賭輸那次都省了。嫌一個一個標麻煩就用 `dynamic=True` 宣布所有維度可變，代價是少了按 shape 特化的機會，kernel 可能慢一些。rank 變動沒有符號可救，只能在進函式前把輸入整成固定的維度數。
