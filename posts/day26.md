...
昨天把執行期的 launch overhead 收完帳，結尾留了一句「shape 要穩定」。其實不只 CUDA Graph 怕變動，整個 torch.compile 都建立在「這張圖的假設還成立」上面。假設一塌，Guard 驗票失敗，Dynamo 就再編一張圖。偶爾一次是機制正常運作，每次呼叫都來一次就是事故，編譯換來的加速全數被編譯本身吃掉。更麻煩的是撞到上限之後整個函式會默默退回 eager，速度掉回原點，還一句錯誤訊息都沒有。今天就來處理這個 Part 4 最常見的事故，重編怎麼發生、怎麼用 log 找到兇手、怎麼對症下藥。

正文開始！

## 重編不是 bug，是機制在補洞

先把後半段機制補完。Guard 的機制前面拆過，Dynamo 在 trace 時把所有假設寫成 Guard，附在編好的成品上掛回 code object。當時只說驗票失敗就重編，但掛在 code object 上的其實不是一張圖，而是一串 cache entry，每個 entry 是一組 Guard 加一份改寫過的 bytecode。呼叫進來時沿著這串 entry 一個一個驗，誰的 Guard 全過就直接執行誰，全都不過才輪到重編，新的 entry 再插進這串清單。所以重編的準確描述是，現有的每一張圖都接不住這組輸入，只好再編一張專屬的，代價就是一次完整編譯，秒級起跳。負責印出驗票失敗現場的程式在 [`torch/_dynamo/guards.py`](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/_dynamo/guards.py)。

值得先記下的是，這串清單不是免費的。每次呼叫都要沿著 entry 逐一驗票，驗到第一個全過的為止，圖越養越多，命中前要白驗的 Guard 也越多。所以重編爆炸的帳有兩筆，明著的是一次次編譯費，暗著的是就算命中，呼叫的固定開銷也被拉長，昨天才剛把執行期 overhead 壓下去，這裡一不小心又全還回來。
...
前八次呼叫總共付了 10.19 秒，養出八張圖，每張只被用過一次就再也派不上用場。這還是一個一行的玩具函式，換成真實模型，每次重編付的就是講 cache 時量過的那種冷編譯價錢，同樣的病灶乘上去有多痛可想而知。然後 call 8 突然只剩 4 ms，不是修好了，是保險絲斷了。
...
同一份程式再做幾組對照，把常見的爆炸源分個類。第一組把同樣的常數改成引數傳入，十個不同的值只重編一次，log 只有一行 `s == 0.0` 的失敗。這是 automatic dynamic 在接，第一次賭輸之後這個純量就升級成符號，一張圖吃下所有值。但它只照顧 frame 的輸入，從 global 撿來的值不在名單上，這就是剛剛炸掉的原因。
...
0 和 1 永遠被特化，這是 automatic dynamic 的既定規則，符號的下界是 2，最後一個 batch 落單剩 1 筆這種事，每次都會多養一張圖。

第三組是全域開關。`no_grad` 裡外交替呼叫同一個函式，log 印出 `GLOBAL_STATE changed: grad_mode`。這其實合理，有沒有 grad 生出來的圖本來就該不一樣。這種重編是有界的，兩張圖各守一邊，之後怎麼交替都有現貨，屬於可以共存的體質，不算爆炸。dtype 和 device 換來換去也是同一類，TENSOR_MATCH 把它們整組押住，混用 float32 和 float64 就是兩張圖，只要種類有限就有限。
