...
不過你的程式就是會碰世界。而這邊的兩個極端都不行：把修改塞進圖，後端的自由沒了；把修改丟掉，程式的語意錯了。所以出路就只有一條：翻譯期不真的改，先記下來；圖跑完，再補做。這其實跟資料庫的 transaction 是同一個思路，所有寫入先進 log，commit 的瞬間一次生效，外界要嘛看到全部、要嘛什麼都沒看到。
...
這裡有一條生死線值得特別拉出來講一下，就是 Existing 與 New 的分野：existing 的修改一定要重播，因為外面的世界看得到這個物件；new 的物件如果沒逃出函式（沒被 return、沒被塞進既有結構），整筆帳直接勾銷，連重建都省了。`ValueMutationExisting` 上還有一個 `is_modified` 旗標，`append` 那一刻才被翻成 True，讓結帳時可以快速濾出「真的被改過」的那些，而不是掃過所有被登記的物件。
...
生成的 bytecode 先呼叫 `__compiled_fn_1` 拿到輸出，然後才開始結帳。仔細看結帳這段的形狀：它先把 `object.__setattr__`、`self`、`'calls'`、`1` 全部排上 stack，中間插進 `log[:] = [1]` 的 slice 賦值，最後才 `CALL 3` 完成屬性寫回。這種「先把所有新值排上 stack、再一筆一筆執行寫入」的兩段式結構待會看原始碼時會再出現。這邊有三個要點，我們先筆記起來：
...
這邊還有一個容易忽略的細節：修改被記帳之後，後續的讀取讀的是帳本，不是真實物件。
...
這邊順便劃清一條邊界：Tensor 的 in-place 運算（`x.add_(1)`）不歸這本帳管。它是 Tensor 運算，直接進圖，之後由 AOTAutograd 的 Functionalization 收拾。`SideEffects` 管的是 Python 層：屬性、容器、全域變數、closure 的 cell。
...
這邊有幾個值得停下來看的細節。list 不是把 `append` 重做一遍，而是整個內容用 slice 賦值換掉，這樣不管中間經歷幾次 `append`、`pop`、`reverse`，重播永遠只有一筆，而且是對原本那個 list 物件動手，別人手上握著的 reference 依然有效。改屬性時特地繞過自訂 `__setattr__` 也是同個邏輯：那個 `__setattr__` 裡的邏輯在翻譯期已經被 inline 追蹤過、效果都記在帳上了，重播時再跑一次就會重複執行。
...
而在結帳之前其實還有一步過濾：`prune_dead_object_new()`。它從「翻譯結束時還活著的東西」出發做可達性分析，根節點有三類：即將被 return 的值（`tx.stack`）、Graph Break 時要留給後半段的區域變數（`symbolic_locals`）、以及所有 existing 物件。從這些根一路走引用鏈，走得到的 new 物件才需要重建，走不到的整筆勾銷。
...
到這邊我們終於可以把三本帳拼起來了。同一個值，讀和寫走的是不同的帳：
...
帳本能記的，前提是 Dynamo 對這個修改建得了模。建不了模的，翻譯就只能舉手 Graph Break。這邊從原始碼裡挑幾個具體的例子：
...
今天的核心其實可以濃縮成一句話：圖只算值，不碰世界；修改在翻譯期進帳本，圖跑完由生成的 bytecode 一次重播最終狀態。帳本內部是 `id_to_variable` 認人、`store_attr_mutations` 記帳、`keepalive` 防止物件被回收，四種 `mutation_type` 裡 Existing 的修改必須重播，沒逃出去的 New 物件由 `prune_dead_object_new` 整筆勾銷。翻譯期間帳本是唯一真相，真實物件凍結到重播那一刻；重播按型別各有套路，list 換內容、dict 做 update、屬性走 `STORE_ATTR`，找回物件靠的是 Source。
