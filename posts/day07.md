...
其中 Existing 與 New 的分野是一條生死線。existing 的修改一定要 replay，因為外面的世界看得到這個物件。new 的物件如果沒逃出函式（沒被 return、沒被塞進既有結構），整筆帳直接勾銷，連重建都省了。`ValueMutationExisting` 上還有一個 `is_modified` 旗標，`append` 那一刻才被翻成 True，讓結帳時可以快速濾出「真的被改過」的那些，而不是掃過所有被登記的物件。
...
生成的 bytecode 先呼叫 `__compiled_fn_1` 拿到輸出，然後才開始結帳。仔細看結帳這段的形狀。它先把 `object.__setattr__`、`self`、`'calls'`、`1` 全部排上 stack，中間插進 `log[:] = [1]` 的 slice 賦值，最後才 `CALL 3` 完成屬性寫回。這種「先把所有新值排上 stack、再一筆一筆執行寫入」的兩段式結構待會看原始碼時會再出現。有三個要點值得先筆記起來。

- **replay 的是最終狀態，不是過程**。迴圈裡 `append` 十次，replay 一次補齊十個元素。`counter += 1` 三次，寫回一次最終值。整張圖的執行是原子的，外界只在圖跑完之後才回來看世界。
...
第三步裡每種型別都有自己的 replay 套路，直接對照原始碼整理成一張表。
...
幾個細節值得停下來多看兩眼。list 不是把 `append` 重做一遍，而是整個內容用 slice 賦值換掉，這樣不管中間經歷幾次 `append`、`pop`、`reverse`，replay 永遠只有一筆，而且是對原本那個 list 物件動手，別人手上握著的 reference 依然有效。改屬性時特地繞過自訂 `__setattr__` 也是同個邏輯。那個 `__setattr__` 裡的邏輯在翻譯期已經被 inline 追蹤過、效果都記在帳上了，replay 時再跑一次就會重複執行。
...
- **replay**：`codegen_update_mutated` 生指令時，要先把「被改的那個物件」放上 stack 才能對它 `STORE_ATTR`，而找回這個物件靠的正是它的 Source，也就是發出 `LOAD_FAST self`、`LOAD_DEREF log` 這些指令的那條鏈。Source 一頭生 Guard、一頭生重建 bytecode，Day 5 埋的這句話，後半句真正的使用者就是 SideEffects。
...
- 對有 `maxlen` 的 `deque` 做修改。滿了會自動擠掉舊元素，replay 「最終狀態」需要知道每筆操作的順序，但帳本只記結果不記過程，v2.8.0 直接放棄（`gb_type="Side effect on existing deque with limited maxlen"`）。
- 在 higher order operator（例如 `torch.cond` 的分支）裡改外層 scope 的變數。分支可能不被執行，replay 與否變成 runtime 才知道的事，`check_allowed_side_effect` 會擋下來。
- 要重建一個沒跑完的 generator，但追蹤期間又有變數被改。Dynamo 得把 generator 耗盡才能重建它，這會多執行到不該執行的修改，只能報錯。

而 Graph Break 本身也跟帳本有關。斷圖的瞬間走的同樣是 `compile_subgraph` 這條收攤路，前半段的帳要先結清，把已記下的修改全部 replay 完，才能把控制權還給 CPython，因為接手的真實 Python 必須看到最新狀態。這是 Graph Break 貴的另一個原因，不只圖被切碎，帳本也被迫提前結算，原本可以合併成一筆的修改被拆成好幾次 replay。

## 結語

今天的核心其實濃縮起來就一句，圖只算值，不碰世界。修改在翻譯期進帳本，圖跑完由生成的 bytecode 一次 replay 最終狀態。帳本內部是 `id_to_variable` 認人、`store_attr_mutations` 記帳、`keepalive` 防止物件被回收，四種 `mutation_type` 裡 Existing 的修改必須 replay，沒逃出去的 New 物件由 `prune_dead_object_new` 整筆勾銷。翻譯期間帳本是唯一真相，真實物件凍結到 replay 那一刻。replay 按型別各有套路，list 換內容、dict 做 update、屬性走 `STORE_ATTR`，找回物件靠的是 Source。
