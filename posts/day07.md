...
到目前為止 Dynamo 手上已經有兩本帳了：Source 記「出處」，也就是每個值在 runtime 要怎麼拿；Guard 記「前提」，也就是這張圖成立的條件。今天講第三本：`SideEffects`，記「修改」。

前兩天被追蹤的程式都很乖：算，然後 return。但真實的 Python 會改東西：`self.counter += 1`、往 list 裡 `append`、寫全域變數、動 closure 的 cell。這些修改進不了圖，因為圖是純函數式的；也丟不得，因為語意會錯。今天看 Dynamo 怎麼走出第三條路：翻譯期一筆一筆記帳，圖跑完再由生成的 bytecode 把帳一次結清。我們會先看帳本內部長什麼樣、實際跑一個會改東西的 `forward` 去讀改寫後的 bytecode、再對照 [`side_effects.py`](https://github.com/pytorch/pytorch/blob/v2.8.0/torch/_dynamo/side_effects.py) 看重播的程式碼是怎麼生出來的，最後把它跟前兩本帳的關係接起來。
...
但你的程式就是會碰世界。兩個極端都不行：把修改塞進圖，後端的自由沒了；把修改丟掉，程式的語意錯了。出路只有一條：翻譯期不真的改，先記下來；圖跑完，再補做。這跟資料庫的 transaction 是同一個思路，所有寫入先進 log，commit 的瞬間一次生效，外界要嘛看到全部、要嘛什麼都沒看到。
...
Existing 與 New 的分野是生死線：existing 的修改一定要重播，因為外面的世界看得到這個物件；new 的物件如果沒逃出函式（沒被 return、沒被塞進既有結構），整筆帳直接勾銷，連重建都省了。`ValueMutationExisting` 上還有一個 `is_modified` 旗標，`append` 那一刻才被翻成 True，讓結帳時可以快速濾出「真的被改過」的那些，而不是掃過所有被登記的物件。
...
生成的 bytecode 先呼叫 `__compiled_fn_1` 拿到輸出，然後才開始結帳。仔細看結帳這段的形狀：它先把 `object.__setattr__`、`self`、`'calls'`、`1` 全部排上 stack，中間插進 `log[:] = [1]` 的 slice 賦值，最後才 `CALL 3` 完成屬性寫回。這種「先把所有新值排上 stack、再一筆一筆執行寫入」的兩段式結構待會看原始碼時會再出現。這裡有三個要點：
...
對照原始碼，這件事就是一對出入口。`STORE_ATTR` 的 handler 最後會走到 `side_effects.store_attr()`，一行 dict 賦值把新替身塞進 `store_attr_mutations[item][name]`；而讀屬性時 `load_attr()` 會先查同一個 dict，查到就直接回傳帳本裡的替身，查不到才去問真實物件。這是被兩個限制夾出來的設計：翻譯期真的去改物件，side effect 就提前洩漏，圖還沒跑世界就變了；但讀到舊值語意又錯。唯一的出路就是讓帳本成為權威，所有讀寫都先過它。

順便劃清一條邊界：Tensor 的 in-place 運算（`x.add_(1)`）不歸這本帳管。它是 Tensor 運算，直接進圖，之後由 AOTAutograd 的 Functionalization 收拾。`SideEffects` 管的是 Python 層：屬性、容器、全域變數、closure 的 cell。

## 重播的 bytecode 是怎麼生出來的
...
有幾個值得停下來看的細節。list 不是把 `append` 重做一遍，而是整個內容用 slice 賦值換掉，這樣不管中間經歷幾次 `append`、`pop`、`reverse`，重播永遠只有一筆，而且是對原本那個 list 物件動手，別人手上握著的 reference 依然有效。改屬性時特地繞過自訂 `__setattr__` 也是同個邏輯：那個 `__setattr__` 裡的邏輯在翻譯期已經被 inline 追蹤過、效果都記在帳上了，重播時再跑一次就會重複執行。
...
結帳前還有一步過濾：`prune_dead_object_new()`。它從「翻譯結束時還活著的東西」出發做可達性分析，根節點有三類：即將被 return 的值（`tx.stack`）、Graph Break 時要留給後半段的區域變數（`symbolic_locals`）、以及所有 existing 物件。從這些根一路走引用鏈，走得到的 new 物件才需要重建，走不到的整筆勾銷。
...
帳本能記的，前提是 Dynamo 對這個修改建得了模。建不了模的，翻譯只能舉手 Graph Break。原始碼裡幾個具體的例子：
...
圖只算值，不碰世界；修改在翻譯期進帳本，圖跑完由生成的 bytecode 一次重播最終狀態。帳本內部是 `id_to_variable` 認人、`store_attr_mutations` 記帳、`keepalive` 防止物件被回收，四種 `mutation_type` 裡 Existing 的修改必須重播，沒逃出去的 New 物件由 `prune_dead_object_new` 整筆勾銷。翻譯期間帳本是唯一真相，真實物件凍結到重播那一刻；重播按型別各有套路，list 換內容、dict 做 update、屬性走 `STORE_ATTR`，找回物件靠的是 Source。
