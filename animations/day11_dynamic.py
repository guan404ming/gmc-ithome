...
class Dynamic(Scene):
    def construct(self):
        ...
        tag1 = T("no entry -> COMPILE #1", font=MONO, font_size=13, color=ACCENT).next_to(b1, UP, buff=0.3)
        ...
        glabel = T("graph #1 · static", font=MONO, font_size=13, color=MUTED).next_to(token, UP, buff=0.32)
        ...
        self.remove(seed)
        self.add(card[1][1][0])
        fs = T("frame_state   x: dim0 = 4  dim1 = 4", font=MONO, font_size=13, color=DIM).next_to(card, DOWN, buff=0.45)
        ...
        lock = card[1][0]
        mm = VGroup(T("size mismatch at index 0", font=MONO, font_size=13, color=ACCENT), T("expected 4, actual 8", font=MONO, font_size=13, color=ACCENT)).arrange(DOWN, aligned_edge=LEFT, buff=0.14).next_to(card, RIGHT, buff=0.45)
        ...
        fsm = T("frame_state   x: dim0 = 4 -> 8", font=MONO, font_size=13, color=MUTED)
        fsm[18:22].set_color(ACCENT)
        fsz = T("會變", font=CJK, font_size=13, color=ACCENT)
        ...
        gl2 = T("graph #2 · dynamic", font=MONO, font_size=13, color=MUTED).move_to(glabel)
        ...
        verify(b3)
        rt = T("reuse, no recompile", font=MONO, font_size=13, color=MUTED).next_to(card2, RIGHT, buff=0.45)
